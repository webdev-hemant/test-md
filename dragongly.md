---
name: Dragonfly Studio Credits Full
overview: "Implement Dragonfly as the fast data layer for studio credit checks across both projects: NitroCloud backend seeds org/member studio data into Dragonfly, and NitroStudio gateway reads from Dragonfly on the hot path with hybrid increment (atomic Dragonfly HINCRBY + write-through to MongoDB)."
todos:
  - id: nc-extend-dragonfly-service
    content: "NitroCloud: Add org + member studio config methods to DragonflyService (HMSET, HSET, DEL, HINCRBY) with TypeScript interfaces"
    status: pending
  - id: nc-hook-org-creation
    content: "NitroCloud: Call setOrgStudioConfig in initializeFreeCreditFields + org create flow"
    status: pending
  - id: nc-hook-member-mgmt
    content: "NitroCloud: Call Dragonfly on addMember, removeMember, updateMemberStudioAccess, updateMemberStudioSettings"
    status: pending
  - id: nc-hook-billing
    content: "NitroCloud: Call Dragonfly on topupStudioCredits, updateSubscriptionPlan, syncSubscriptionFromCheckout, stripe webhooks, updateStudioConfig"
    status: pending
  - id: gw-config-env
    content: "Gateway: Add Dragonfly config fields + env vars to config.go and .env"
    status: pending
  - id: gw-dragonfly-store
    content: "Gateway: Create internal/store/dragonfly.go with rueidis client + HGETALL/HINCRBY methods"
    status: pending
  - id: gw-studio-access-middleware
    content: "Gateway: Modify studio_access.go to read from Dragonfly with MongoDB fallback"
    status: pending
  - id: gw-billing-middleware
    content: "Gateway: Modify billing.go for hybrid increment (Dragonfly HINCRBY + MongoDB write-through)"
    status: pending
  - id: gw-main-init
    content: "Gateway: Initialize DragonflyStore in main.go, wire into middleware, graceful shutdown"
    status: pending
  - id: gw-go-mod
    content: "Gateway: Add github.com/redis/rueidis to go.mod"
    status: pending
isProject: false
---
